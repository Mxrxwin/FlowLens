...
function TimeRangeControl({ value, onChange }: Props) {
  ...
  const rangeInvalid =
    ...
    !!draft.manualFrom &&
    !!draft.manualTo &&
    draft.manualTo.toDate(tz) <= draft.manualFrom.toDate(tz);

  const canApply =
    draft.mode === "preset" ||
    (!!draft.manualFrom && !!draft.manualTo && !rangeInvalid);
    ...
    <Popover isOpen={open} onOpenChange={setOpen} placement="bottom-end">
      <PopoverTrigger>
        <Button
          ...
          fullWidth
          disableRipple
          className="h-12 px-3 justify-start font-normal !bg-[#F4F4F5]"
        >
          ...
          <div className="grid grid-cols-2 gap-4">
            <div className="space-y-3">
              ...
              <DatePicker
                ...
                granularity="day"
                hideTimeZone
                minValue={(draft.manualFrom as any) ?? undefined}
                maxValue={today(getLocalTimeZone())}
                placeholderValue={midnightPlaceholder}
